import type { CsvParseResult } from "@typed_cli/shared";
...
async function main() {
  ...
  if (!fs.existsSync(outputPath)) {
    ...
    console.error(
      "Run cli-csv first: node apps/cli-csv/dist/index.js < data/input.csv",
    );
    process.exit(1);
  }
  ...
  const data: CsvParseResult = JSON.parse(jsonText);

  const isMapFlagPresent = Boolean(process.argv.indexOf("--map"));

  if (!isMapFlagPresent) {
    console.log(
      "To transform the JSON please ensure to include the --map flag",
    );
  }
  ...
  if (!mapCommands) {
    console.log(
      "You have to specify at least one map using the format: oldName:newName",
    );
  }
  ...
  commands.map((command) => {
    ...
    dataToModify.map((dataItem) => {
      if (oldValue in dataItem) {
        dataItem[newValue] = dataItem[oldValue as keyof typeof dataItem];
        delete dataItem[oldValue as keyof typeof dataItem];
      }
    });
  });
